...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='trackingmambav2', help='training script name')
    parser.add_argument('--config', type=str, default='TrackingmambaV2-ep150-full-256', help='yaml configure file name')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "/root/anaconda3/envs/trackingmambav2/bin/python3.8 lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "python -m torch.distributed.run --nproc_per_node %d --master_port %d lib/train/run_training.py " \
        ...
        train_cmd = "/root/anaconda3/envs/trackingmambav2/bin/python3.8 -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
