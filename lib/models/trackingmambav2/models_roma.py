...
class ARM(nn.Module):
    ...
    def forward_features(self, z, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        #x = (hidden_states[:, :token_position, :].mean(1)+hidden_states[:, token_position+1:, :].mean(1))/2.
    # else:
        #y = hidden_states[:, token_position, :]
        aux_dict = {"attn": None}
        return hidden_states, aux_dict



    def forward(self, z, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        x, aux_dict = self.forward_features(z, x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
        # if return_features:
        #     return x
        # x = self.head(x)
        return x, aux_dict
