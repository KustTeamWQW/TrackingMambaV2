...
from lib.models.trackingmambav2.mamba_simple import Mamba
from mamba_ssm.utils.generation import GenerationMixin
from mamba_ssm.utils.hf import load_config_hf, load_state_dict_hf

from lib.models.trackingmambav2.rope import *
import random
...
def _expand_v3_pretrained_state_dict(state_dict):
    ...
    for layer_idx in range(24):
        ...
        branch_pairs = (
            ("A_log", "A_c_log"),
            ("D", "D_c"),
            ("conv1d.weight", "conv1d_c.weight"),
            ("conv1d.bias", "conv1d_c.bias"),
            ("x_proj.weight", "x_proj_c.weight"),
            ("dt_proj.weight", "dt_proj_c.weight"),
            ("dt_proj.bias", "dt_proj_c.bias"),
            ("A_b_log", "A_c_b_log"),
            ("D_b", "D_c_b"),
            ("conv1d_b.weight", "conv1d_c_b.weight"),
            ("conv1d_b.bias", "conv1d_c_b.bias"),
            ("x_proj_b.weight", "x_proj_c_b.weight"),
            ("dt_proj_b.weight", "dt_proj_c_b.weight"),
            ("dt_proj_b.bias", "dt_proj_c_b.bias"),
        )
        for src_name, dst_name in branch_pairs:
            src, dst = prefix + src_name, prefix + dst_name
            if src in state_dict and dst not in state_dict:
                state_dict[dst] = state_dict[src].clone()
    return state_dict


def _load_vim_pretrained(model, pretrained):
    checkpoint = torch.load(pretrained, map_location="cpu")
    state_dict = checkpoint.get("model", checkpoint)
    state_dict = _expand_v3_pretrained_state_dict(state_dict)
    if "pos_embed" in state_dict:
        pos_embed = state_dict.pop("pos_embed")
        if pos_embed.ndim == 3:
            patch_pos = pos_embed[:, 1:] if pos_embed.shape[1] in (197, 730) else pos_embed
            num_patch = patch_pos.shape[1]
            side = int(math.isqrt(num_patch))
            if side * side == num_patch:
                grid = patch_pos.reshape(1, side, side, patch_pos.shape[-1]).permute(0, 3, 1, 2)
                pos_z = F.interpolate(grid, size=(8, 8), mode="bicubic", align_corners=False)
                pos_x = F.interpolate(grid, size=(16, 16), mode="bicubic", align_corners=False)
                state_dict["pos_embed_z"] = pos_z.permute(0, 2, 3, 1).reshape(1, 64, patch_pos.shape[-1])
                state_dict["pos_embed_x"] = pos_x.permute(0, 2, 3, 1).reshape(1, 256, patch_pos.shape[-1])

    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
    print('Load pretrained model from: ' + pretrained)
    print(f"Backbone pretrained load: missing={len(missing_keys)}, unexpected={len(unexpected_keys)}")
...
class EinFFT(nn.Module):
    def __init__(self, dim):
        ...
        self.complex_weight_1 = nn.Parameter(
            torch.empty(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32))
        self.complex_weight_2 = nn.Parameter(
            torch.empty(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32))
        self.complex_bias_1 = nn.Parameter(
            torch.empty(2, self.num_blocks, self.block_size, dtype=torch.float32))
        self.complex_bias_2 = nn.Parameter(
            torch.empty(2, self.num_blocks, self.block_size, dtype=torch.float32))
        self.reset_parameters()

    def reset_parameters(self):
        for weight in (self.complex_weight_1, self.complex_weight_2):
            nn.init.xavier_uniform_(weight[0])
            nn.init.xavier_uniform_(weight[1])
            with torch.no_grad():
                weight.mul_(1.0 / math.sqrt(2.0))
        nn.init.zeros_(self.complex_bias_1)
        nn.init.zeros_(self.complex_bias_2)
    ...
    def forward(self, x):
        ...
        threshold = self.sparsity_threshold.abs()
...
class Block(nn.Module):
    def __init__(
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0.0001 else nn.Identity()
    ...
    def forward(
        ...
        hidden_states_mlp = hidden_states + self.drop_path(self.mlp(self.norm2(hidden_states)))
        hidden_states = self.alpha * hidden_states + (1 - self.alpha) * hidden_states_mlp
        return hidden_states, residual

    def set_token_layout(self, z_len, x_len):
        self.mixer.z_len = z_len
        self.mixer.x_len = x_len
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, z, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        x = self.patch_embed(x)                  #x.shape = torch.Size([B, 3, 256, 256])  -> torch.Size([b, 256, 384])
        z = self.patch_embed(z)                  #z.shape = torch.Size([B, 3, 128, 128])  -> torch.Size([b, 64, 384])
        B, M, _ = x.shape
        z_len, x_len = z.shape[1], x.shape[1]
        ...
        if not self.if_bidirectional:                                 # True
            for layer in self.layers:
                layer.set_token_layout(z_len, x_len)
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
...
def vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, stride=8, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    
    if pretrained:
        _load_vim_pretrained(model, pretrained)
    return model

@register_model
def vim_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, stride=8, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
