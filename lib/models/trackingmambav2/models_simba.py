...
class EinFFT(nn.Module):
    ...
    def forward(self, x, H, W, H_z, W_z, i):
        # s_size_x = [4096, 1024, 256, 64]
        # s_size_z = [1024, 256, 64, 16]
        # z, x = torch.split(x, [s_size_z[i], s_size_x[i]], dim=1)
        B, N, C = x.shape
...
class Block_mamba(nn.Module):
    ...
    def forward(self, x, z, H, W, H_z, W_z, i):
        s_size_x = [4096, 1024, 256, 64]
        s_size_z = [1024, 256, 64, 16]
        x = torch.cat((z, x), dim=1)
        x = x + self.drop_path(self.attn(x))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W, H_z, W_z, i))
        z, x = torch.split(x, [s_size_z[i], s_size_x[i]], dim=1)
        return x, z
...
class SiMBA(nn.Module):
    def __init__(self,
                 ...
                 embed_dims_x=[64, 128, 320, 448],
                 embed_dims_z=[64, 128, 320, 448],
                 mlp_ratios=[8, 8, 4, 4],
                 drop_path_rate=0.,
                 norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3],
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed_x = Stem(in_chans, stem_hidden_dim, embed_dims_x[i])
                patch_embed_z = Stem(in_chans, stem_hidden_dim, embed_dims_z[i])
            else:
                patch_embed = DownSamples(embed_dims_x[i - 1], embed_dims[i])
                patch_embed_x = DownSamples(embed_dims_x[i - 1], embed_dims_x[i])
                patch_embed_z = DownSamples(embed_dims_z[i - 1], embed_dims_z[i])
            ...
            setattr(self, f"patch_embed_x{i + 1}", patch_embed_x)
            setattr(self, f"patch_embed_z{i + 1}", patch_embed_z)
    ...
    def forward(self, z, x):
        if not self.return_dense:
            ...
            z, H_z, W_z = self.forward_embeddings(z)
            ...
            x, aux_dict = self.forward_tokens(x, z, H, W, H_z, W_z)
            # x_cls = self.head(x[:, 0])
            # x_aux = self.aux_head(
            #     x[:, 1:]
    ...
    def forward_tokens(self, x, z, H, W, H_z, W_z):
        B = x.shape[0]
        ...
        for i in range(self.num_stages):
            if i != 0:
                patch_embed_x = getattr(self, f"patch_embed_x{i + 1}")
                patch_embed_z = getattr(self, f"patch_embed_z{i + 1}")
                x, H, W = patch_embed_x(x)
                z, H_z, W_z = patch_embed_z(z)
            block = getattr(self, f"block{i + 1}")
            for blk in block:
                    x, z = blk(x, z, H, W, H_z, W_z, i)
            #if i != self.num_stages - 1:
            x_blk = x
            z_blk = z
            norm = getattr(self, f"norm{i + 1}")
        ...
        # norm = getattr(self, f"norm{self.num_stages}")
        # x = norm(x)
        x_blk = torch.cat((z_blk, x_blk), dim=1)
        aux_dict = {"attn": None}
        return x_blk, aux_dict
...
def simba_b(pretrained=False, **kwargs):
    model = SiMBA(
        ...
        embed_dims_x=[64, 128, 320, 512],
        embed_dims_z=[64, 128, 320, 512],
        mlp_ratios=[8, 8, 4, 4],
        drop_path_rate=0.1,
        clip_grad=None,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
...
def simba_l(pretrained=False, **kwargs):
    model = SiMBA(
        ...
        embed_dims_x=[96, 192, 384, 512],
        embed_dims_z=[96, 192, 384, 512],
        mlp_ratios=[8, 8, 4, 4],
        drop_path_rate=0.3,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        depths=[3, 6, 18, 3],
...
if __name__ == '__main__':
    # pretrained = '/home/z/projects/pythonprojects2/TrackingMamba_MambaVision/mambavision_base_1k.pth.tar'
    # pretrained = '/home/z/pretrained_models/hustvlVim-small-midclstok+/vim_s_midclstok_80p5acc.pth'
    # pretrained = '/home/z/pretrained_models/hustvlVim-small-midclstok+/vim_s_midclstok_80p5acc.pth'
    pretrained = '/home/z/projects/pythonprojects2/TrackingMambaV2_2/pre/checkpoint-195.pth.tar'
    # model = VisionMamba(
    ...
    # net = create_model(
    #     model_name="mamba_vision_L3_256_21k"
    # )
